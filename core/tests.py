...
class WooCommerceSyncTests(TestCase):
    ...
    @patch("core.woocommerce.request.urlopen")
    def test_json_request_retries_with_query_auth_after_cloudfront_html(self, mock_urlopen):
        class HtmlResponse:
            def __enter__(self):
                return self
            ...
            def read(self):
                return b"<!DOCTYPE HTML><TITLE>ERROR</TITLE> Generated by cloudfront (CloudFront)"
        ...
        class JsonResponse:
            ...
            def read(self):
                return b'[{"id":501,"sku":"ENQ-501"}]'

        mock_urlopen.side_effect = [HtmlResponse(), JsonResponse()]

        response = _json_request("products", params={"per_page": 1})

        self.assertEqual(response, [{"id": 501, "sku": "ENQ-501"}])
        first_request = mock_urlopen.call_args_list[0].args[0]
        second_request = mock_urlopen.call_args_list[1].args[0]
        self.assertTrue(first_request.get_header("Authorization").startswith("Basic "))
        self.assertIsNone(second_request.get_header("Authorization"))
        self.assertIn("consumer_key=ck_test", second_request.full_url)
        self.assertIn("consumer_secret=cs_test", second_request.full_url)
    ...
    @patch("core.woocommerce.request.urlopen")
    def test_json_request_reports_non_json_response_without_crashing(self, mock_urlopen):
        class HtmlResponse:
            def __enter__(self):
                return self
        ...
        mock_urlopen.return_value = HtmlResponse()

        with self.assertRaises(WooCommerceAPIError) as captured:
            _json_request("products")

        self.assertIn("CloudFront returned a non-JSON page", str(captured.exception))
        self.assertNotIn("<!DOCTYPE", str(captured.exception))
    ...
    @patch("core.woocommerce.request.urlopen")
    def test_json_request_sends_user_agent_and_sanitizes_cloudfront_403(self, mock_urlopen):
        def cloudfront_error():
            return HTTPError(
                url="https://shop.example.com/wp-json/wc/v3/products",
                code=403,
                msg="Forbidden",
                hdrs=None,
                fp=BytesIO(b"<!DOCTYPE HTML><TITLE>ERROR</TITLE> Generated by cloudfront (CloudFront)"),
            )

        mock_urlopen.side_effect = [cloudfront_error(), cloudfront_error()]

        with self.assertRaises(WooCommerceAPIError) as captured:
            _json_request("products")

        request_obj = mock_urlopen.call_args.args[0]
        self.assertEqual(request_obj.get_header("User-agent"), WOOCOMMERCE_USER_AGENT)
        self.assertIn("CloudFront blocked the request", str(captured.exception))
        self.assertNotIn("<!DOCTYPE", str(captured.exception))
            ...
            [
                {
                    ...
                    "target": {
                        ...
                        "package_name": "com.mathukai.operations",
